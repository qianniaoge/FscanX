...
func NETBIOS(info *config.HostData)(error,bool,string){
	...
	msg += fmt.Sprintf("%s \n=> IP:%s \n=> NAME:[%s\\%s] \n=> FQDN:[%s]\n=> OS:[%s]\n",isdc,info.HostName, nbname.group, nbname.unique,nbname.fqdn,nbname.osversion)

	if len(nbname.group) > 0 || len(nbname.unique) > 0 {
		return errors.New("scan ok by netbios"),true,msg
	}

	return errors.New("scan failed by netbios"),false,""
}
...
func netbios1(info *config.HostData)(nbname NbnsName,err error){
	...
	for index < start+offset+length {
		...
		if string(item_type) == "\x07\x00" {
			...
			// 这里输出FQDN名方便后续的zerlogon的攻击测试

			if NetBIOS_ITEM_TYPE[string(item_type)] == "DNS computer name"{
				nbname.fqdn = string(item_content)
			}
